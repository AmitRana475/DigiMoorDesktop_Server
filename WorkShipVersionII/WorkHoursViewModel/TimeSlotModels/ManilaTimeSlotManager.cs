...
{
    public class ManilaTimeSlotManager : CommanConditionsTimeSlot, ITimeSlotManager
    {
        private readonly ShipmentContaxt sc;
        public ManilaTimeSlotManager()
        {
            if (sc == null)
            {
        ...
        //public override void UpdateSqlCe(WorkHoursClass ob)
        //{
        //    base.UpdateSqlCe(ob);
        //}
        ...
        {
            base.GetbackhoursValues(obj, WorkHoursList, CheckIDL);
        }
        ...
        public void TimeSlotPageLoadMethod(WorkHoursClass obj, List<WorkHoursClass> WorkHoursList, CrewDetailClass CrewDetailList, Grid DynamicGrid, DefineRulesClass DefineRules, string Opa90Rule, List<InternationalClass> CheckIDL, DateTime? MaxFutureDate)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(lbl1))
                {
                    ...
                    while (iDatebm < 150)
                    {
                        ...
                        if (checkuser != null)
                        {
                            ...
                            {
                                int countbms1 = checkuser.Cellcount;
                                int finalCellcount = 2016 - cellcouns;
                                string startEndbbs = checkuser.hrs;
                                string[] st22bbs = startEndbbs.TrimEnd(',').Split(',');
                    ...
                    for (int y1 = 0; y1 < today.Length; y1++)
                    {
                        ...
                        else
                        {
                            ...
                            //RaisePropertyChanged("CommonData1");
                            obj.RestHours = Convert.ToDecimal(txtRH);
                            obj.RestHourAny24 = txtRH.ToString();
                            ...
                            string[] numsbms1 = st4bms42.Skip(star1).Take(336).ToArray();
                            string[] numsbms2 = st4bms42.Skip(star2).Take(336).ToArray();
                            string[] numsbms3 = st4bms42.Skip(star3).Take(336).ToArray();
                            string[] numsbms4 = st4bms42.Skip(star4).Take(336).ToArray();
                            string[] numsbms5 = st4bms42.Skip(star5).Take(336).ToArray();
                            string[] numsbms6 = st4bms42.Skip(star6).Take(336).ToArray();
                ...
                SaveNC1(obj);
                //CommonData1.NonConfirmities = obj.NonConfirmities;
