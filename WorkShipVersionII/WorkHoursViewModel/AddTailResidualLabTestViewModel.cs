...
{
    public class AddTailResidualLabTestViewModel : ViewModelBase
    {

        private readonly ShipmentContaxt sc;

        TextInfo textinfo = new CultureInfo("en-US", false).TextInfo;
        public AddTailResidualLabTestViewModel(ResidualLabTestClass edeps)
        {
        ...
        public AddTailResidualLabTestViewModel()
        {
            StaticHelper.Editing = true;
        ...
        public void GetRopeType()
        {
            ...
            ropetype.Clear();
            RopeTypeCombo1 rop;
            SqlDataAdapter adp = new SqlDataAdapter("select Id, UniqueID +' - '+ CertificateNumber as CertificateNumber  from MooringRopeDetail where DeleteStatus=0 and RopeTail=1 and OutofServiceDate is null order by UniqueID asc", sc.con);
            ...
            try
            {
                ...
                if (CheckErrorMessage.CheckErrorMessages)
                {
                    ...
                    if (ResLbTst.TestResults <= 75)
                    {
                        ...
                        var NotiMsg = "RopeTail " + ropename + " - " + uniqueid + " residual strength Current - " + minvalue + "% / Min. Required 75%";
                                          //var NotiMsg = "RopeTail residual strength below minimum allowable (" + minvalue + " - / Required - 75%)";
                    ...
                    try
                    {
                        using (SqlDataAdapter adp = new SqlDataAdapter("InsertRopeAttachment", sc.con))
                        {
                            ...
                            adp.SelectCommand.Parameters.AddWithValue("@LineResidual", "ResidualRopeTail");
