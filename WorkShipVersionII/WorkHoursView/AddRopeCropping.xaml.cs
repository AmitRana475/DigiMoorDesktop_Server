...
namespace WorkShipVersionII.WorkHoursView
{
    ...
    /// </summary>
    public partial class AddRopeCropping : UserControl
    {
        private readonly ShipmentContaxt sc;
        public AddRopeCropping()
        {
        ...
        private void comboRope_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                ...
                var data = sc.AssignRopetoWinch.Where(x => x.RopeId == AddRopeCroppingViewModel.sropetype.Id && x.IsActive == true).FirstOrDefault();
                if (data != null)
                {
                    int winchid = data.WinchId;
                    AddRopeCroppingViewModel._ropecroppingClass.WinchId = winchid;
                ...
                {
                    AddRopeCroppingViewModel._ropecroppingClass.WinchId = 0;
                    txtOutboard.Text = "Not Assigned";
                    txtAssignedLocation.Text = "Not Assigned";
                    txtAssignedWinch.Text = "Not Assigned";
                }
                var crplength = sc.RopeCropping.Where(x => x.RopeId == AddRopeCroppingViewModel.sropetype.Id && x.IsActive == true).Select(x => x.LengthofCroppedRope).Sum();
        ...
        private void txtcrpdrope_TextChanged(object sender, TextChangedEventArgs e)
        {
            try
            {
                ...
                int ropeid = AddRopeCroppingViewModel.sropetype.Id;
                ...
                if(check > ss)
                {
                    MessageBox.Show("You cannot crop the line beyond its total length of " + ss + " mtrs  !", "Line Cropping", MessageBoxButton.OK, MessageBoxImage.Warning);

                    //This Line Cropping length cannot be more than the line length of {Line length} mtrs
                ...
                if (dt1.Rows.Count > 0 && dt1.Rows[0][0] != DBNull.Value)
                {
                    ...
                    {
                        MessageBox.Show("You cannot crop the line beyond its total length of " + ss + " mtrs  !", "Line Cropping", MessageBoxButton.OK, MessageBoxImage.Warning);
                       // MessageBox.Show("You are not cropped line more than "+ss+" !", "Character Length", MessageBoxButton.OK, MessageBoxImage.Warning);
                        txtcrpdrope.Text = "";
                    }
                }
        ...
        private void Dpdateofshifting_CalendarClosed(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (dateto > datefrom)
                {
                    MessageBox.Show("Cropping Date can not be less than Line Received Date!", "Line Cropping", MessageBoxButton.OK, MessageBoxImage.Error);
